...
from ultralytics.nn.extra_modules.block import PyramidPoolAgg
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Conv, RepC3, CSEIB):
            ...
            if m in (RepC3, CSEIB):
                ...
                n = 1
        elif m in (AIFI,TransformerEncoderLayer_PFET):
            c2 = ch[f]
            args = [ch[f], *args]
        ...
        if isinstance(c2, list) and m not in {SGMLFN}:
            is_backbone = True
            m_ = m
            m_.backbone = True
        else:
