...
class BaseTrainer:
    ...
    def build_dataset(self, img_path, mode='train', batch=None):
        ...
        # 构建数据集配置
        data_dict = self.data if hasattr(self, 'data') and self.data is not None else {}
        
        # 确保img_path在data_dict中
        if isinstance(img_path, str) and img_path not in data_dict:
            data_dict['path'] = img_path
        
        # 创建数据集
        dataset = build_yolo_dataset(
            data=data_dict,
            batch=batch or self.args.batch if hasattr(self.args, 'batch') else 16,
            imgsz=self.args.imgsz if hasattr(self.args, 'imgsz') else 640,
            augment=mode == 'train',
            hyp=self.args if hasattr(self.args, 'hsv_h') else {},  # 超参数
            rect=self.args.rect if hasattr(self.args, 'rect') else False,
